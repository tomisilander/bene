...
void init_scorer(const char *essarg, const char *logregfile)
{
  ...
  if ((0 == strncmp(essarg, "BIC", 3)) || (0 == strncmp(essarg, "AIC", 3)) || (0 == strncmp(essarg, "HQC", 3)))
  {
                  ...
                  const char *logregfile, const char *selfile, int use_subset_walker)
{
  ...
  else
  {
    ...
    sel_vars = (int *)calloc(nof_vars, sizeof(int));
    {
      ...
      if (data_count <= max_datacount)
        cread_data(datfile);
    }
    /* init_globals_for_selvars and scorer in walker*/
  }
  if (cstrfile)
    get_constraints(cstrfile);
}
...
int main(int argc, char *argv[])
{
  ...
  void *options = gopt_sort(&argc, (const char **)argv, gopt_start(gopt_option('l', GOPT_ARG, gopt_shorts('l'), gopt_longs("logregfile")), gopt_option('n', GOPT_ARG, gopt_shorts(0), gopt_longs("nof-tasks")), gopt_option('i', GOPT_ARG, gopt_shorts(0), gopt_longs("task-index")), gopt_option('c', GOPT_ARG, gopt_shorts('c'), gopt_longs("constraints")), gopt_option('p', GOPT_ARG, gopt_shorts('p'), gopt_longs("prior")), gopt_option('s', GOPT_ARG, gopt_shorts('s'), gopt_longs("selectvars")), gopt_option('b', GOPT_NOARG, gopt_shorts(0), gopt_longs("no-save-all")), gopt_option('x', GOPT_NOARG, gopt_shorts(0), gopt_longs("no-recurse")), gopt_option('w', GOPT_NOARG, gopt_shorts(0), gopt_longs("use-subset-walker")), gopt_option('m', GOPT_ARG, gopt_shorts('m'), gopt_longs("max-parents"))));
  if (argc != 5)
  {
    fprintf(stderr,
  ...
  else
  {
    ...
    if (gopt_arg(options, 'n', &nof_tasks_s))
      nof_tasks = atoi(nof_tasks_s);
    if (gopt_arg(options, 'i', &task_index_s))
      task_index = atoi(task_index_s);
    if (gopt_arg(options, 'c', &cstrfile))
    {
    }; /* could check file etc. */
    if (gopt_arg(options, 'p', &priorfile))
    {
    }; /* could check file etc. */
    if (gopt_arg(options, 'l', &logregfile))
    {
    }; /* could check file etc. */
    if (gopt_arg(options, 's', &selfile))
    {
    }; /* could check file */
    if (gopt_arg(options, 'm', &max_parents_s))
      max_parents = atoi(max_parents_s);
    if (gopt_arg(options, 'b', &tmp))
      save_all_scores = 0;
    if (gopt_arg(options, 'x', &tmp))
      no_recurse = 1;
    if (gopt_arg(options, 'w', &tmp))
      use_subset_walker = 1;
